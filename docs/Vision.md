...
* **Current extreme-quantization boundary ([`AES-QNT-010`](../CAPABILITY_LEDGER.md) `partial`)**: IQ2_XXS now has canonical 66-byte GGML host decoding/GEMM with the upstream grid and an independent raw-block oracle regression. Real-model and CUDA evidence remain open. IQ1_S and the custom ternary descriptor still reject execution without mutation.
