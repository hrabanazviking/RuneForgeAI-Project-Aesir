...
* **Stage 5.3 Operational CLI Milestone ([`AES-CLI-005`](../CAPABILITY_LEDGER.md) `partial`)**: Native `create`, `list`/`ls`, `show`, `cp`, `rm`, and `verify` use the restart-safe catalog. Explicit model-byte import and verification work; `ps` and garbage collection remain fail closed until their ownership models exist.
* **Stage 5.2 Model Store Milestone ([`AES-CLI-004`](../CAPABILITY_LEDGER.md) `partial`)**: Implemented validated manifests, a caller-configured relative store root, a versioned atomic Linux catalog, and explicit immutable SHA-256 blob ingestion/verification. Automatic pull registration and crash-safe garbage collection remain targets.
...
12. **Complete Ollama Terminal Command Suite & CLI Compatibility ([`AES-CLI-005`](../CAPABILITY_LEDGER.md)):**
   ...
   - **The Scroll & Vault of Mímisbrunnr (`ModelManifest`, `DurableModelStore`):** Restart-safe recipe catalog plus explicit measured SHA-256 model-byte ingestion and verification; automatic pull registration and garbage collection remain unfinished.
