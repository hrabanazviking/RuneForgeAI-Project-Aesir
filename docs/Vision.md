...
* **Current external quantization boundary ([`AES-QNT-009`](../CAPABILITY_LEDGER.md) `partial`)**: AutoGPTQ 4-bit/8-bit and AutoAWQ GEMM 4-bit have validated metadata-bearing host dequantization/GEMM primitives with hand-calculated packed-word tests. Model loading, tensor attachment, CUDA dispatch, external-fixture parity, and full-model execution remain open. EXL2, HQQ, and SmoothQuant continue to fail closed.
...
13. **Universal Compressed LLM Format Matrix:**
   ...
   - **External quantization boundary:** AutoGPTQ 4-bit/8-bit and AutoAWQ GEMM 4-bit have checked host primitives over their real packed-weight, zero, scale, and group metadata; GPTQ also supports optional `g_idx`. Loader integration, CUDA execution, and external-fixture parity are not yet implemented; EXL2, HQQ, and SmoothQuant still reject execution.
