...
* **Current status ([`AES-ECO-008`](../CAPABILITY_LEDGER.md) `verified` for the arithmetic primitive)**: `SpeculativeEngine.evaluate_acceptance()` validates aligned probabilities and explicit draws, then computes the sequential `min(1, p_target/p_draft)` accepted prefix. Draft/target execution, residual correction sampling, KV mutation, distribution parity and speedup remain unavailable; legacy approximation paths raise.
...
* **Stage 67.1 Model Store Guard Milestone ([`AES-CLI-005`](../CAPABILITY_LEDGER.md) `partial`)**: `remove_model_checked()` rejects missing and caller-identified active recipes. The broader capability now has built-CLI restart evidence, while a real live process registry and model-byte lifecycle remain unfinished.
...
* **Current experimental status ([`AES-SYS-001`](../CAPABILITY_LEDGER.md) `missing`)**: Configuration can record CIA/WIC/NSFI/MQARI intent, but supported model commands reject it. The former checksum, cosine-wave, sine/fractal-weight and harmonic transforms were synthetic stand-ins and have been removed; all four execution surfaces now fail without mutation. Each proposal needs separate scientific, model-equivalence and physical-performance evidence.
...
### Current boundary — quantized hardware autotuning unavailable (AES-QNT-011)
...
* **Current extreme-quantization boundary ([`AES-QNT-010`](../CAPABILITY_LEDGER.md) `partial`)**: IQ2_XXS and IQ1_S now have canonical 66-byte and 50-byte GGML host decoding/GEMM with their complete upstream grids and independent raw-block oracle regressions. Real-model and CUDA evidence remain open. The custom ternary descriptor still rejects execution without mutation.
* **Current external quantization boundary ([`AES-QNT-009`](../CAPABILITY_LEDGER.md) `partial`)**: AutoGPTQ 4-bit/8-bit, AutoAWQ GEMM 4-bit, EXL2 mixed 2/3/4/5/6/8-bit, static SmoothQuant/torch-int W8A8, and native HQQ 4-bit `axis=1` have validated metadata-bearing host dequantization/GEMM primitives with hand-calculated tests. Model loading, tensor attachment, CUDA dispatch, external-fixture parity, and full-model execution remain open. Other HQQ variants and dynamic SmoothQuant variants continue to fail closed.
...
* **Current Q8/FP8 boundary ([`AES-QNT-005`](../CAPABILITY_LEDGER.md) `verified`)**: Q8_0 reads canonical 34-byte GGML blocks and Q8_1 reads canonical 40-byte blocks with F32 scale/sum metadata. Raw hand-calculated cases cover both plus representative finite E4M3 and E5M2 values. Stored-model Q8_1 and broad FP8 fixture compatibility are not claimed.
...
* **Current compute boundary ([`AES-CPU-001`](../CAPABILITY_LEDGER.md) & [`AES-QNT-002`](../CAPABILITY_LEDGER.md))**: Activation and exact-block dequantizers retain bounded empty-input behavior. Fixed-scale byte helpers, including the former `dequantize_q2_k()`, were removed; the compressed dispatcher now requires complete blocks and valid storage.
...
* **Current speculative boundary ([`AES-ECO-008`](../CAPABILITY_LEDGER.md))**: The old pointer/logit shortcut raises unsupported. Only validated probability-ratio prefix arithmetic is implemented; no default acceptance is fabricated.
* **Current GBNF boundary ([`AES-ECO-007`](../CAPABILITY_LEDGER.md))**: Token-text boolean/number masking validates pointers and candidates; the older ID-only masking signature raises unsupported and leaves logits unchanged.
...
* **Stage 5.3 Operational CLI Milestone ([`AES-CLI-005`](../CAPABILITY_LEDGER.md) `partial`)**: Native `create`, `list`/`ls`, `show`, `cp`, and `rm` use the restart-safe recipe catalog. `ps` remains fail closed until a live process registry exists; model-byte lifecycle is separate.
...
12. **Complete Ollama Terminal Command Suite & CLI Compatibility ([`AES-CLI-005`](../CAPABILITY_LEDGER.md)):**
   ...
   - **The Scroll & Vault of Mímisbrunnr (`ModelManifest`, `DurableModelStore`):** Restart-safe recipe catalog with non-cryptographic Modelfile fingerprints; content-addressed model-byte storage remains unfinished.
...
13. **Universal Compressed LLM Format Matrix:**
   ...
   - **External quantization boundary:** AutoGPTQ 4-bit/8-bit, AutoAWQ GEMM 4-bit, EXL2 mixed 2/3/4/5/6/8-bit, static SmoothQuant W8A8, and native HQQ 4-bit `axis=1` have checked host primitives over their real weight, scale, group, and permutation metadata; GPTQ also supports optional `g_idx`. Loader integration, CUDA execution, and external-fixture parity are not yet implemented; the remaining HQQ/SmoothQuant variants still reject execution.
...
14. **Universal Multi-Engine Ecosystem Matrix:**
   ...
   - **Bounded grammar masks (`GBNFGrammar`):** Actual decoded token text is supported for booleans and JSON numbers only; general GBNF/JSON/regex generation is unavailable.
   - **Speculative acceptance (`SpeculativeEngine`):** Validated probability-ratio prefix arithmetic exists; draft/target execution, residual sampling and KV integration do not.
