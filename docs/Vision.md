...
* **Stage 5.2 Model Store Milestone ([`AES-CLI-004`](../CAPABILITY_LEDGER.md) `partial`)**: Implemented validated manifests, a caller-configured relative store root, a versioned atomic Linux catalog, explicit immutable SHA-256 blob ingestion/verification, and pinned pull registration. Crash-safe garbage collection remains a target.
...
12. **Complete Ollama Terminal Command Suite & CLI Compatibility ([`AES-CLI-005`](../CAPABILITY_LEDGER.md)):**
   ...
   - **The Scroll & Vault of Mímisbrunnr (`ModelManifest`, `DurableModelStore`):** Restart-safe catalog plus explicit measured SHA-256 model-byte ingestion, verification, and pinned pull registration; garbage collection remains unfinished.
