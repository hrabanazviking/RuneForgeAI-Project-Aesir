...
* **Stage 5.3 Operational CLI Milestone ([`AES-CLI-005`](../CAPABILITY_LEDGER.md) `partial`)**: Native `create`, `list`/`ls`, `show`, `cp`, `rm`, `verify`, and locked reference-aware `gc` use the restart-safe catalog. `ps` remains fail closed until a live-session ownership model exists.
* **Stage 5.2 Model Store Milestone ([`AES-CLI-004`](../CAPABILITY_LEDGER.md) `partial`)**: Implemented validated manifests, a caller-configured relative store root, a versioned atomic Linux catalog, explicit immutable SHA-256 blob ingestion/verification, pinned pull registration, and validate-before-delete garbage collection. Systematic crash and I/O-fault recovery evidence remains a target.
...
12. **Complete Ollama Terminal Command Suite & CLI Compatibility ([`AES-CLI-005`](../CAPABILITY_LEDGER.md)):**
   ...
   - **The Scroll & Vault of Mímisbrunnr (`ModelManifest`, `DurableModelStore`):** Restart-safe catalog plus explicit measured SHA-256 model-byte ingestion, verification, pinned pull registration, and locked reference-aware garbage collection.
