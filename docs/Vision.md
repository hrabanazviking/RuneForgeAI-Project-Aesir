...
* **Current external quantization boundary ([`AES-QNT-009`](../CAPABILITY_LEDGER.md) `partial`)**: AutoGPTQ 4-bit has a validated metadata-bearing host dequantization/GEMM primitive with hand-calculated packed-word tests. Model loading, tensor attachment, CUDA dispatch, external-fixture parity, and full-model execution remain open. GPTQ 8-bit, AWQ, EXL2, HQQ, and SmoothQuant continue to fail closed.
...
13. **Universal Compressed LLM Format Matrix:**
   ...
   - **External quantization boundary:** AutoGPTQ 4-bit has a checked host primitive over its real packed-weight, zero, scale, group, and optional `g_idx` metadata. Loader integration, CUDA execution, and external-fixture parity are not yet implemented; GPTQ 8-bit, AWQ, EXL2, HQQ, and SmoothQuant still reject execution.
