...
* **Current external quantization boundary ([`AES-QNT-009`](../CAPABILITY_LEDGER.md) `partial`)**: AutoGPTQ 4-bit/8-bit, AutoAWQ GEMM 4-bit, static SmoothQuant/torch-int W8A8, and native HQQ 4-bit `axis=1` have validated metadata-bearing host dequantization/GEMM primitives with hand-calculated tests. Model loading, tensor attachment, CUDA dispatch, external-fixture parity, and full-model execution remain open. EXL2, other HQQ variants, and dynamic SmoothQuant variants continue to fail closed.
...
13. **Universal Compressed LLM Format Matrix:**
   ...
   - **External quantization boundary:** AutoGPTQ 4-bit/8-bit, AutoAWQ GEMM 4-bit, static SmoothQuant W8A8, and native HQQ 4-bit `axis=1` have checked host primitives over their real weight and scale metadata; GPTQ also supports optional `g_idx`. Loader integration, CUDA execution, and external-fixture parity are not yet implemented; EXL2 and the remaining HQQ/SmoothQuant variants still reject execution.
