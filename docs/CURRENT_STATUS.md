...
| Conversation crash recovery | Optional `--autosave-dir` chat generations commit exact-token state through synced immutable snapshots and a checksummed atomic manifest. SIGKILL recovery restores the prior committed turn, reports interrupted generation/turn identity, enforces caller retention, preserves unrelated files, and rejects corruption. Healthy idle turn rejection discards only uncommitted intent; interrupted prefill requires `/clear`; unsafe or post-close failures require restart without guessing commit state. |
| Model-switch failure recovery | Targets fail before unload when validation rejects them, preserving the current session. Same-PID target startup carries the previous reference; allocation/load/startup failure after unload gives an exact previous-model restart instruction and never claims automatic rollback. Autosave-bound sessions still reject model switching. |
...
| Native content-addressed store | `create --model` imports measured immutable SHA-256 blobs, pinned `pull --name` registers verified Hub bytes, `verify` rehashes stored content, and locked `gc` validates the complete namespace before reclaiming unreachable blobs and stale stages. One-way legacy migration, immutable catalog backup, and validate-before-replace restore share the same lock and atomic publication path. Built-binary tests cover six concurrent writers, corruption, missing blobs, rollback, backup non-overwrite, and invalid-restore refusal. A Linux process harness kills catalog commits after write, file sync, rename and directory sync; every restart exposes exactly the old or complete new record and remains writable. This is not sudden-power-loss proof; see the [model-store contract](MODEL_STORE.md). |
| Automated checks | The counted suite reports 185 passed, 0 failed, and 1 explicit external-fixture skip (186 total). Physical CUDA/model checks remain opt-in; hosted CI does not claim GPU execution. |
...
| Measured host quantization tuning | A caller-invoked Linux host tuner measures fused packed GEMM against dequantize-then-F16 GEMM, rejects numerically divergent candidates, publishes output only after success, and caches the winner by caller device key plus exact format/shape. A checksummed v1 codec and locked private-stage/fsync/rename/fsync storage gateway support transactional restart restore. Model dispatch and accelerator candidates remain open. |
| Host paged K/V pool | `PagedKVCache` maps multiple logical sequences onto a bounded preallocated physical page pool with owner maps, per-layer initialized lengths, checked exhaustion, tail/sequence release, and reuse. Current CPU/CUDA model attention still uses contiguous caches; scheduling, eviction, prefix sharing, copy-on-write, GPU pages, and real-model integration remain open. |
...
- Host quantization tuning is an explicit API, not automatic inference dispatch.
  Its timings describe only the current process and exact shape. A versioned
  codec and atomic Linux storage gateway exist, but trustworthy automatic
  build/device fingerprinting, statistical benchmarking, metadata-bearing
  formats, CUDA candidates, and other physical backends remain unfinished.
