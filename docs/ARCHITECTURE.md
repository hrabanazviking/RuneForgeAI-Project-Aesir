...
- **Multi-Device Topology & RAG:** Holds `knowledge_base: MimirStore` and `topology: DeviceTopology` (initialized via `num_devices`). In `generate()` and `generate_stream()`, if `knowledge_base.count > 0`, invokes `extract_query_embedding(prompt, hidden_dim)` to compute mean-pooled query vectors via `token_embd.weight` lookup or string hash projection (`AES-RAG-003`), executes `knowledge_base.search_knn(query_vector, 3)`, and prepends retrieved document context (`[CONTEXT]: ...`) to the prompt before tokenization. Executes `forward_pass()` passing `topology` to drive single- or multi-device sharded matrix operations across the Bifrost Shard Matrix.
