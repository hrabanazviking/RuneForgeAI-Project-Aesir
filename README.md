...
* **CLI & Transport:** Single-shot CPU execution and the native CUDA `run`/`chat` path work for their documented model profiles. Restart-safe catalog commands are operational: `create --model` imports exact bytes into an immutable SHA-256-addressed blob store, `verify` rehashes the stored inode, and pinned `pull --name` registers a verified Hub artifact. Garbage collection, authentication/resume, `ps`/`stop`, and compatibility APIs remain unfinished. See the [model-store guide](docs/MODEL_STORE.md), [current status](docs/CURRENT_STATUS.md), and ledger.
