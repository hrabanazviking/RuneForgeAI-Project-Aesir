...
|---|---|---|---|
| **Valhalla (CLI)** | `aesir_engine/cli/INTERFACE.md` | Single-shot generation CLI (`run`), Modelfile parser, Operational catalog CLI (`list`, `show`, `ps`, `create`, `cp`, `rm`), Interactive REPL (`RuneREPL`), Flag options (`CLIOptions`) | Verified single-shot, Modelfile multiline parser, store persistence, operational CLI, REPL & CLI flag options (`AES-CLI-001`, `AES-CLI-003`, `AES-CLI-004`, `AES-CLI-005`, `AES-CLI-008`, `AES-CLI-009` `verified`) |
