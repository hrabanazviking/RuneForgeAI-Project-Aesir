...
Runtime expansion note: CPU Q2_K, Q3_K, Q4_K, Q5_K, Q6_K, IQ1_S, and IQ2_XXS execution
...
- **Implementation evidence:** `CompressedFormatType` retains reserved internal names while `GGMLType.to_compressed_format` maps implemented GGML quantized tensor IDs 2, 3, 6–14, IQ2_XXS ID 16, and IQ1_S ID 19 (excluding unsupported Q8_K 15) and raises for every other value.
...
- **Implementation evidence:** `core/extreme_quantization.mojo` decodes canonical 66-byte GGML IQ2_XXS and 50-byte IQ1_S blocks. IQ2_XXS uses the upstream 256-entry grid, parity-completed sign masks, and local scale fields. IQ1_S uses the 2,048-entry ternary grid, split 11-bit indices, odd local scale multipliers, and signed delta. Both enforce exact storage and provide F32-accumulating GEMM; GGUF types 16 and 19 are admitted. `scripts/generate_iq_codebooks.py` reproducibly derives both checked-in tables from `gguf-py`. The custom ternary descriptor remains an explicit mutation-free error.
- **Executable evidence:** `E-MASTER` cases `quantization.iq2_xxs_known_value` and `quantization.iq1_s_known_value` each check 32 oracle values, the full 256-value sum, fused GEMM, and short-storage rejection against raw blocks generated by the independent pinned `gguf-py` decoder. Case `quantization.ternary_boundary` preserves strict refusal coverage.
- **Evidence boundary:** Verified canonical IQ2_XXS and IQ1_S host block decoding and matrix multiplication. No IQ model fixture, CUDA kernel, full-model execution, or performance evidence exists yet. `TERNARY_155BIT` is not implemented.
- **Next acceptance gate:** Add pinned real IQ1_S/IQ2_XXS row or model fixtures and CUDA kernels; define a versioned native ternary block contract before admitting that descriptor.
- **Audit:** IQ1_S and IQ2_XXS canonical host primitive milestones, September 1, 2026.
