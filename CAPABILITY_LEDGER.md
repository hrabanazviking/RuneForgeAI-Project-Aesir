...
- **Implementation evidence:** `core/external_quantization.mojo` implements canonical host matrix contracts for AutoGPTQ 4-bit/8-bit, AutoAWQ GEMM 4-bit, and SmoothQuant/torch-int static per-tensor W8A8. GPTQ uses input-channel-packed UInt32 weights and zero-minus-one values; AWQ uses its `[0,2,4,6,1,3,5,7]` packed output order and direct zero points. SmoothQuant carries signed INT8 weights, calibrated positive input/weight scales, optional FP32 bias, symmetric nearest/saturating activation quantization, INT32 accumulation, and the combined-scale epilogue. Every view has exact backing-storage counts and metadata validation. `core/compute.mojo` exposes metadata-aware overloads while legacy byte-only/RuneTensor entry points fail closed. EXL2 and HQQ remain mutation-free boundaries.
- **Executable evidence:** `E-MASTER` cases `quantization.gptq_4bit_known_value`, `quantization.gptq_8bit_known_value`, `quantization.awq_4bit_known_value`, and `quantization.smoothquant_int8_known_value` check independently hand-calculated dequantized values and GEMM rows. The SmoothQuant case additionally covers signed rounding and saturation. The AWQ case proves its reorder map; GPTQ 4-bit covers activation-order groups and fail-before-mutation metadata rejection. Cases `quantization.external_dequant_boundaries`, `quantization.exl2_boundary`, and `quantization.hqq_boundary` retain strict refusal coverage.
- **Evidence boundary:** Verified bounded AutoGPTQ, AutoAWQ GEMM, and one explicit SmoothQuant/torch-int static per-tensor W8A8 host primitive. There is no artifact loader, metadata attachment to `RuneTensor`, CUDA kernel for these layouts, external artifact fixture, or full-model parity evidence yet. Dynamic/per-token and INT8-output SmoothQuant variants are not implemented. No implementation claim is made for EXL2 or HQQ.
- **Next acceptance gate:** Wire parsed GPTQ/AWQ/SmoothQuant metadata into model tensors, add external-fixture parity, and then add measured CUDA kernels.
