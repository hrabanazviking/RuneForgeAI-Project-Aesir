...
- **Implementation evidence:** `core/external_quantization.mojo` implements canonical host matrix contracts for AutoGPTQ 4-bit/8-bit and AutoAWQ GEMM 4-bit. GPTQ uses input-channel-packed UInt32 weights and zero-minus-one values; AWQ uses its `[0,2,4,6,1,3,5,7]` packed output order and direct zero points. All views carry per-group/per-output F16 scales, exact backing-storage counts, checked metadata, F16 dequantization, and F32-accumulating GEMM; GPTQ also supports optional activation-order `g_idx`. `core/compute.mojo` exposes metadata-aware overloads while legacy byte-only/RuneTensor entry points fail closed. EXL2, HQQ, and SmoothQuant remain mutation-free boundaries.
- **Executable evidence:** `E-MASTER` cases `quantization.gptq_4bit_known_value`, `quantization.gptq_8bit_known_value`, and `quantization.awq_4bit_known_value` check raw packed words, independently hand-calculated dequantized values, and two GEMM rows. The AWQ case assigns distinct logical output values to prove its reorder map. The GPTQ 4-bit case additionally covers activation-order groups, exact output bounds, and fail-before-mutation invalid metadata. Cases `quantization.external_dequant_boundaries`, `quantization.exl2_boundary`, `quantization.hqq_boundary`, and `quantization.smoothquant_int8_boundary` retain strict refusal coverage.
- **Evidence boundary:** Verified bounded AutoGPTQ 4-bit/8-bit and AutoAWQ GEMM 4-bit host primitives. There is no safetensors/config loader, metadata attachment to `RuneTensor`, CUDA kernel for these layouts, external artifact fixture, or full-model parity evidence yet. No implementation claim is made for EXL2, HQQ, or SmoothQuant.
- **Next acceptance gate:** Wire parsed GPTQ/AWQ metadata into model tensors, add external-fixture parity, and then add measured CUDA kernels.
