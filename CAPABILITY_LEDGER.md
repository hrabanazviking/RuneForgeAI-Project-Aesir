...
Runtime expansion note: CPU Q2_K, Q3_K, Q4_K, Q5_K, Q6_K, and IQ2_XXS execution
...
- **Implementation evidence:** `CompressedFormatType` retains reserved internal names while `GGMLType.to_compressed_format` maps implemented GGML quantized tensor IDs 2, 3, 6–14 and IQ2_XXS ID 16 (excluding unsupported Q8_K 15) and raises for every other value.
- **Executable evidence:** `E-MASTER` case `quantization.enum` in `test_quantization.mojo`.
- **Evidence boundary:** Verified stable descriptor names, exact supported mappings, and strict rejection of F32 and the still-unsupported upstream IQ1_S type. Descriptor presence does not imply execution support.
...
- **Implementation evidence:** `core/extreme_quantization.mojo` decodes canonical 66-byte GGML IQ2_XXS blocks with the upstream 256-entry grid, parity-completed sign masks, group scale fields, F16 block scale, exact storage checks, and F32-accumulating GEMM. GGUF type 16 is admitted through the loader whitelist. IQ1_S and the custom ternary descriptor remain explicit mutation-free errors.
- **Executable evidence:** `E-MASTER` case `quantization.iq2_xxs_known_value` checks 32 oracle values, the full 256-value sum, fused GEMM, and short-storage rejection against a raw block generated by the independent pinned `gguf-py` decoder. Cases `quantization.iq1_s_boundary` and `quantization.ternary_boundary` preserve strict refusal coverage.
- **Evidence boundary:** Verified canonical IQ2_XXS host block decoding and matrix multiplication. No IQ2_XXS model fixture, CUDA kernel, full-model execution, or performance evidence exists yet. IQ1_S and `TERNARY_155BIT` are not implemented.
- **Next acceptance gate:** Add a pinned real IQ2_XXS row/model fixture and CUDA kernel; implement IQ1_S from its authoritative grid; define a versioned native ternary block contract before admitting that descriptor.
- **Audit:** IQ2_XXS canonical host primitive milestone, September 1, 2026.
