...
- **Implementation evidence:** `core/external_quantization.mojo` implements canonical host matrix contracts for AutoGPTQ 4-bit/8-bit, AutoAWQ GEMM 4-bit, SmoothQuant/torch-int static per-tensor W8A8, and native HQQ 4-bit `axis=1`. HQQ follows the upstream cross-group packing: the first half of flattened group rows occupy high nibbles and the second half occupy low nibbles, with floating zero/scale metadata. SmoothQuant carries signed INT8 weights and activations with INT32 accumulation. GPTQ and AWQ retain their distinct packing and zero conventions. Every view has exact backing-storage counts and metadata validation. `core/compute.mojo` exposes metadata-aware overloads while legacy byte-only/RuneTensor entry points fail closed. EXL2 remains a mutation-free boundary.
- **Executable evidence:** `E-MASTER` cases `quantization.gptq_4bit_known_value`, `quantization.gptq_8bit_known_value`, `quantization.awq_4bit_known_value`, `quantization.smoothquant_int8_known_value`, and `quantization.hqq_4bit_axis1_known_value` check independently hand-calculated dequantized values and GEMM rows. The HQQ case uses deliberately different high/low group rows; SmoothQuant covers signed rounding and saturation; AWQ proves its reorder map; GPTQ 4-bit covers activation-order groups and fail-before-mutation metadata rejection. Cases `quantization.external_dequant_boundaries` and `quantization.exl2_boundary` retain strict refusal coverage.
- **Evidence boundary:** Verified bounded AutoGPTQ, AutoAWQ GEMM, static per-tensor SmoothQuant W8A8, and HQQ 4-bit `axis=1` host primitives. There is no artifact loader, metadata attachment to `RuneTensor`, CUDA kernel for these layouts, external artifact fixture, or full-model parity evidence yet. Other HQQ bits/axes and dynamic/per-token or INT8-output SmoothQuant variants are not implemented. No implementation claim is made for EXL2.
- **Next acceptance gate:** Wire parsed GPTQ/AWQ/HQQ/SmoothQuant metadata into model tensors, add external-fixture parity, and then add measured CUDA kernels.
