...
**Ledger version:** Application S14 conversation crash-recovery autosave, September 22, 2026
...
|---|---|---|
| `E-MASTER` | `pixi run mojo run --target-accelerator sm_89 aesir_engine/tests/run_all.mojo` | 184 named executable cases pass, zero fail, 1 external-fixture case is explicitly skipped, total 185, process exit 0. Synthetic/scaffold cases prove only their narrow local assertions. |
...
- **Implementation evidence:** `cli/conversation.mojo` defines a bounded, FNV-1a-checksummed v1 record containing model SHA-256, profile, context, system and sampling identities, sampler draw position, exact committed token IDs, and readable turns. Save and Markdown export use owner-private exclusive creation, full writes, file sync, and parent-directory sync; load uses bounded no-follow reads. Both CUDA session types record tokens only after successful `forward`, clear them with KV reset, and restore validated tokens into an empty session without sampling or text retokenization. `/save`, `/load`, `/new`, and `/export` are interactive between-turn controls. `scripts/conversations.py` adds a locked named library over those snapshots: NFC Unicode names, random stable entry IDs, exact metadata-to-snapshot validation, atomic save/rename metadata, restart listing, compatibility-filtered open, and exclusive Markdown export. `cli/conversation_autosave.mojo` adds opt-in private-directory generations: synced inflight intent, immutable synced snapshots, a checksummed atomic current manifest, manifest-derived retention and compatible native restore. CUDA chat begins intent before generation and commits only after exact session state and the readable turn agree; reset/load/sampling mutations also checkpoint.
- **Executable evidence:** `E-MASTER` cases `cli.conversation_codec`, `cli.conversation_compatibility`, and `cli.conversation_autosave_generations` cover Unicode/newline round trips, exact token preservation, checksum corruption, unsnapshotted-turn rejection, model/context mismatch refusal, latest-generation recovery, bounded retention, interruption detection and unrelated-file preservation. `E-CONVERSATIONS` covers the named lifecycle in fresh processes. `E-AUTOSAVE` SIGKILLs a native writer after durable turn-two intent and proves a fresh process recovers committed turn one, reports generation/turn two as interrupted, performs owned cleanup, and fails closed on manifest corruption. A previously generated native Qwen snapshot also passed named import, restart discovery and compatibility-open against its recorded identity. Physical Gemma 4 E2B and Qwen 3 0.6B sessions each generated, saved, reset to position zero, reloaded to the exact saved position, and retained the saved turn count; Gemma then continued as turn two with the prior answer in context.
- **Evidence boundary:** Snapshots and autosave restore conversation KV and repetition/sampling sequence only for the same exact model bytes, native profile, context, system prompt, and sampling settings. Named-library save imports an already created snapshot; `open` reports its validated path while native compatibility remains authoritative. Autosave requires a caller-created owner-held private directory, is Linux-specific, and rejects model switching in this slice. The process-kill harness proves visibility and recovery under SIGKILL on the tested filesystem, not sudden-power-loss durability at every syscall or a physical post-crash CUDA continuation. No operation overwrites an existing named entry or export. There is no in-chat library menu, delete command, encrypted storage, or cross-context migration. Named-library interrupted stages are ignored rather than collected automatically.
- **Next acceptance gate:** S15 deterministic recovery after generation errors and failed model switches, then an in-chat library selector/delete policy and physical sampled continuation replay oracle for both session families.
- **Audit:** Round 2 physical and counted verification, 2026-09-11.
