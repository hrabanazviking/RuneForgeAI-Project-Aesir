...
- **Implementation evidence:** `core/external_quantization.mojo` implements the canonical AutoGPTQ 4-bit and 8-bit packed matrix contracts: input-channel-packed UInt32 weights, separately packed zero-minus-one values at the correct bit width, per-group/per-output F16 scales, optional activation-order `g_idx`, exact backing-storage counts, checked metadata, F16 dequantization, and F32-accumulating GEMM. `core/compute.mojo` exposes metadata-aware dequantization and GEMM overloads for both widths. Legacy byte-only/RuneTensor entry points still fail closed because they cannot carry the required metadata. AWQ, EXL2, HQQ, and SmoothQuant remain mutation-free boundaries.
- **Executable evidence:** `E-MASTER` cases `quantization.gptq_4bit_known_value` and `quantization.gptq_8bit_known_value` check raw packed words, independently hand-calculated dequantized values, and two GEMM rows. The 4-bit case additionally covers activation-order groups, exact output bounds, and fail-before-mutation invalid metadata. Cases `quantization.external_dequant_boundaries`, `quantization.awq_4bit_boundary`, `quantization.exl2_boundary`, `quantization.hqq_boundary`, and `quantization.smoothquant_int8_boundary` retain strict refusal coverage.
- **Evidence boundary:** Verified the AutoGPTQ 4-bit and 8-bit host primitives and bounded metadata validation. There is no safetensors/config loader, metadata attachment to `RuneTensor`, CUDA GPTQ kernel, external artifact fixture, or full-model parity evidence yet. No implementation claim is made for the other listed formats.
- **Next acceptance gate:** Wire parsed GPTQ metadata into model tensors and add external-fixture parity, then implement and verify AWQ's distinct packing before adding CUDA kernels.
