...
**Ledger version:** Application S13 named conversation library, September 22, 2026
...
- **Implementation evidence:** `cli/conversation.mojo` defines a bounded, FNV-1a-checksummed v1 record containing model SHA-256, profile, context, system and sampling identities, sampler draw position, exact committed token IDs, and readable turns. Save and Markdown export use owner-private exclusive creation, full writes, file sync, and parent-directory sync; load uses bounded no-follow reads. Both CUDA session types record tokens only after successful `forward`, clear them with KV reset, and restore validated tokens into an empty session without sampling or text retokenization. `/save`, `/load`, `/new`, and `/export` are interactive between-turn controls. `scripts/conversations.py` adds a locked named library over those snapshots: NFC Unicode names, random stable entry IDs, exact metadata-to-snapshot validation, atomic save/rename metadata, restart listing, compatibility-filtered open, and exclusive Markdown export.
- **Executable evidence:** `E-MASTER` cases `cli.conversation_codec` and `cli.conversation_compatibility` cover Unicode/newline round trips, exact token preservation, checksum corruption, unsnapshotted-turn rejection, and model/context mismatch refusal. `E-CONVERSATIONS` covers the named lifecycle in fresh processes; a previously generated native Qwen snapshot also passed import, restart discovery and compatibility-open against its recorded identity. Physical Gemma 4 E2B and Qwen 3 0.6B sessions each generated, saved, reset to position zero, reloaded to the exact saved position, and retained the saved turn count; Gemma then continued as turn two with the prior answer in context.
- **Evidence boundary:** Snapshots restore conversation KV and repetition/sampling sequence only for the same exact model bytes, native profile, context, system prompt, and sampling settings. Named-library save imports an already created snapshot; `open` reports its validated path and can require model/profile/context identity, while the native `/load` performs the full system/sampling compatibility check and KV restore. No operation overwrites an existing named entry or export. There is no in-chat library menu, delete command, encrypted storage, cross-context migration, automatic autosave, or crash injection at each filesystem syscall. Interrupted save staging directories are ignored rather than collected automatically.
- **Next acceptance gate:** S14 atomic-generation autosave and crash recovery, then an in-chat library selector/delete policy and physical sampled continuation replay oracle for both session families.
- **Audit:** Round 2 physical and counted verification, 2026-09-11.
