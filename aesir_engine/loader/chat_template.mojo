...
struct RuneChatTemplate(Copyable):
    ...
    def escape_control_tokens(content: String) -> String:
        """Sanitizes raw prompt control tokens from message content payloads to prevent prompt injection."""
        var res = content
        if "<|im_start|>" in res:
            res = res.replace("<|im_start|>", "[im_start]")
        if "<|im_end|>" in res:
            res = res.replace("<|im_end|>", "[im_end]")
        if "<|start_header_id|>" in res:
            res = res.replace("<|start_header_id|>", "[start_header_id]")
        if "<|end_header_id|>" in res:
            res = res.replace("<|end_header_id|>", "[end_header_id]")
        if "<|eot_id|>" in res:
            res = res.replace("<|eot_id|>", "[eot_id]")
        if "[INST]" in res:
            res = res.replace("[INST]", "(INST)")
        if "[/INST]" in res:
            res = res.replace("[/INST]", "(/INST)")
        if "<<SYS>>" in res:
