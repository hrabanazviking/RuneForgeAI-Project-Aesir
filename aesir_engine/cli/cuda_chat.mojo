...
from aesir import Gemma4CUDASession
...
def dispatch_cuda_chat(args: List[String]) raises:
    if len(args) < 2:
        raise Error("usage: aesir chat <gemma4.gguf> --accel cuda [--prompts file] [--log file] [--max-tokens 16384] [--context 32768] [--system text]")
    ...
    if acceleration != "cuda":
        raise Error("Gemma 4 chat requires explicit --accel cuda; CPU fallback is disabled")
