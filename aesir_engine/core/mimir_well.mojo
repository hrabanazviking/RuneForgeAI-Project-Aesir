...
struct GPUBuffer(Copyable, ImplicitlyCopyable):
    ...
    def validate_zero_copy_contract(self) raises:
        ...
        if self.handle_fd <= 0:
            raise Error(
                "GPUBuffer zero-copy contract unverified: host buffer lacks"
...
def shard_split_gqa_heads(
    ...
    if num_heads % num_devices != 0:
        raise Error(
            "shard_split_gqa_heads: num_heads must be divisible by num_devices"
        )
    if num_kv_heads % num_devices != 0:
        raise Error(
            "shard_split_gqa_heads: num_kv_heads must be divisible by"
            " num_devices"
        )
