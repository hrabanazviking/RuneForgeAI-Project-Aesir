...
Bounded host page manager over preallocated K/V physical blocks. Each logical
sequence has its own page table and per-layer initialized length. Physical pages
carry explicit owner/logical-index metadata, allocate on contiguous writes, and
return to the shared pool on tail or sequence release. Reads translate through
the page table and reject unwritten layer positions before touching storage.

```mojo
struct PagedKVCache(Copyable):
    def __init__(out self, max_seq_len: Int, hidden_dim: Int, mut well: MimirWell, num_layers: Int = 32, block_size: Int = 16, max_sequences: Int = 1, physical_blocks: Int = 0) raises: ...
    def allocate_block(mut self) raises -> Int: ...
    def allocate_block(mut self, sequence_id: Int, logical_block: Int) raises -> Int: ...
    def free_block(mut self, block_idx: Int) raises: ...
    def free_sequence_block(mut self, sequence_id: Int, logical_block: Int) raises: ...
    ...
    def append(mut self, sequence_id: Int, layer_idx: Int, pos: Int, key: RuneTensor[f16], val: RuneTensor[f16]) raises: ...
    def get_k(self, sequence_id: Int, layer_idx: Int, pos: Int, column: Int) raises -> Scalar[f16]: ...
    def get_v(self, sequence_id: Int, layer_idx: Int, pos: Int, column: Int) raises -> Scalar[f16]: ...
    def copy(self) -> Self: ...
```

`physical_blocks=0` preallocates every logical page; a smaller positive value
deliberately overcommits the shared pool and produces a checked exhaustion error.
`copy()` creates an independent K/V and page-metadata snapshot in O(physical
storage + metadata) time and space; it is intended for explicit checkpoints,
not the token hot path. The original cache storage remains owned by `MimirWell`,
while a copied snapshot owns and frees its private allocation.
This host primitive is not yet the cache used by model attention sessions and
does not implement eviction, prefix sharing, copy-on-write, or GPU pages.
...
struct SmoothQuantW8A8Matrix(Copyable, ImplicitlyCopyable):
    # Bounded signed INT8 weights, static scales, and optional FP32 bias.
    ...

struct HQQ4BitAxis1Matrix(Copyable, ImplicitlyCopyable):
    # Native cross-group nibble packing with floating scale and zero metadata.
    ...

struct EXL2Matrix(Copyable, ImplicitlyCopyable):
    # Canonical unshuffled mixed-bit weights, scale metadata, groups, and
    # mutually checked activation-order permutations.
    ...
...
def dequantize_exl2(matrix: EXL2Matrix, out_ptr: Pointer[Float16, MutUntrackedOrigin], output_elements: Int) raises: ...
def gemm_exl2(input: Pointer[Float16, MutUntrackedOrigin], input_elements: Int, input_rows: Int, matrix: EXL2Matrix, output: Pointer[Float16, MutUntrackedOrigin], output_elements: Int) raises: ...
def dequantize_smoothquant_int8(matrix: SmoothQuantW8A8Matrix, out_ptr: Pointer[Float16, MutUntrackedOrigin], output_elements: Int) raises: ...
def gemm_smoothquant_int8(input: Pointer[Float16, MutUntrackedOrigin], input_elements: Int, input_rows: Int, matrix: SmoothQuantW8A8Matrix, output: Pointer[Float16, MutUntrackedOrigin], output_elements: Int) raises: ...
def dequantize_hqq(matrix: HQQ4BitAxis1Matrix, out_ptr: Pointer[Float16, MutUntrackedOrigin], output_elements: Int) raises: ...
def gemm_hqq(input: Pointer[Float16, MutUntrackedOrigin], input_elements: Int, input_rows: Int, matrix: HQQ4BitAxis1Matrix, output: Pointer[Float16, MutUntrackedOrigin], output_elements: Int) raises: ...
```

The views implement AutoGPTQ's 4-bit/8-bit packing and zero-minus-one rule plus
AutoAWQ GEMM's 4-bit output reorder and direct zero points. The SmoothQuant view
implements static per-tensor signed W8A8 with INT32 accumulation and optional
FP32 bias. EXL2 implements the official mixed 2/3/4/5/6/8-bit packing, packed
