...
- The runner registers 49 executable named cases and one explicit skip in a
  deterministic order.
- `TestLedger.finish(50)` prints `[SUMMARY]` pass/fail/skip/total/status keys and
  raises after reporting if any case failed or the total is not 50.
